...
export default function Symptoms() {
  ...
  const questions = useMemo(() => ['What is the main health problem or symptom you want help with?', ...(track?.questions || []), 'Are you using any medicines, vitamins, inhalers, injections, or home remedies now? Type “none” if you are not using any.', 'What is the medicine name and how are you using it? Start typing to choose it from the hospital medicine list, or write “none”.'], [track]);
  ...
  function next() {
    ...
    const result = [...answers, value]; setAnswers(result); setAnswer(''); setMedicineName(''); setSuggestions([]); setNotice('');
    if (result.length >= 10) setStage('review');
  }
  async function save() {
