...
export default function Symptoms() {
  ...
  const questions = interviewQuestions.length ? interviewQuestions : [initialQuestion];
  ...
  function start() { localStorage.setItem(LANGUAGE_STORAGE_KEY, language); window.dispatchEvent(new Event('aarogyavaani-language')); setAnswers([]); setQuestionIndex(0); setInterviewQuestions([initialQuestion]); setAnswer(''); setMedicineName(''); setNotice(''); setStage('interview'); }
  function speak() {
  ...
  function next() {
    ...
    if (current === 0) {
      const focused = trackFor(value);
      setInterviewQuestions([initialQuestion, ...focused.questions, 'Are you using any medicines, vitamins, inhalers, injections, or home remedies now? Type “none” if you are not using any.', 'What is the medicine name and how are you using it? Start typing to choose it from the hospital medicine list, or write “none”.']);
    }
  ...
  async function save() {
    ...
    const combined = answers.map((item, index) => `${interviewQuestions[index] || 'Additional information'}\n${item}`).join('\n\n');
